...
"syn match log4jDate      "\v^(\d{4}\-\d{2}\-\d{2}\s\d{2}(\:\d{2}){2},\d+)\s+" contains=log4jErrorlevel,log4jLogger
"syn match log4jErrorlevel contained " DEBUG "
"syn match log4jLogger contained "\v\s\[.{-}\]"
"
syn match log4jText ".*" contains=log4jDate,log4jLogger
syn match log4jDate contained "\v^(\d{4}\-\d{2}\-\d{2}\s\d{2}(\:\d{2}){2},\d+)\s\w+" contains=log4jErrorlevel 
syn match log4jErrorlevel contained "\(DEBUG\|ERROR\|FATAL\|INFO\|TRACE\|WARN\) " 
syn match log4jLogger contained "\v\[\w+\..{-}\]"
syn case match
